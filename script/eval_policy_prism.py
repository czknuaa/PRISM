import sys
sys.path.insert(0, './policy/PRISM-Policy/PRISM-Policy')
sys.path.append('./')
...
@hydra.main(
    ...
    config_path=str(pathlib.Path(__file__).parent.joinpath(
        '../../policy/PRISM-Policy/PRISM-Policy/prism', 'config'))
)
...
def main(cfg):
    ...
    task_cfg = read_taskcfg_from_yaml(cfg.raw_task_name)
    print('[DBSCAN]: eps:',task_cfg['DBSCAN']['eps'],'  min_samples: ', task_cfg['DBSCAN']['min_samples'], ' is_use_color: ', task_cfg['DBSCAN']['is_use_color'])
    args['expert_seed'] = seed
    args['expert_data_num'] = expert_data_num

    task = class_decorator(args['task_name'])

    st_seed = 100000 * (1+seed) + 10
    suc_nums = [] 
    test_num = 100
    topk = 1
    
    prism = PRISM(cfg, checkpoint_num)

    st_seed, suc_num = test_policy(task, args, prism, st_seed, test_num=test_num)
    suc_nums.append(suc_num)

    topk_success_rate = sorted(suc_nums, reverse=True)[:topk]
    if not cfg.policy.use_pc_color: # result_prism/task_camera/expert_data_num/ckpt_seed
        save_dir  = f'eval_result/prism/{TASK}_{head_camera_type}/{expert_data_num}'
    else:
        save_dir = f'eval_result/prism_w_rgb/{TASK}_{head_camera_type}/{expert_data_num}'
...
def test_policy(Demo_class, args, prism, st_seed, test_num=20):
    global TASK
    epid = 0      
    seed_list=[]  
    suc_num = 0   
    ...
    while succ_seed < test_num:
        ...
        Demo_class.setup_demo(now_ep_num=now_id, seed = now_seed, is_test = True, ** args)
        Demo_class.apply_prism(prism, args)

        now_id += 1
        Demo_class.close()
        if Demo_class.render_freq:
            Demo_class.viewer.close()
        prism.env_runner.reset_obs()
