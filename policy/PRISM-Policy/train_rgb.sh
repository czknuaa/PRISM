...
if [ ! -d "./data/${task_name}_${head_camera_type}_${expert_data_num}.zarr" ]; then
    echo "zarr does not exist, run pkl2zarr_prism.py"
    cd ../..
    ...
    if [ ! -d "./data/${task_name}_${head_camera_type}_pkl/episode${expert_data_num_minus_one}" ]; then
        ...
        python script/pkl2zarr_prism.py ${task_name} ${head_camera_type} ${expert_data_num}
        cd policy/PRISM-Policy
    fi
fi

bash scripts/train_policy_rgb.sh robot_prism ${task_name} ${head_camera_type} ${expert_data_num} train ${seed} ${gpu_id}
